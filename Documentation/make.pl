...
sub putContStr
# 引数で渡された文字列をそのまま出力する。
# ただし、# を含む行の場合は、以下のように出力を行なう。
# input : コマンド内容  # 説明  # 説明の日本語訳
# output: コマンド内容  # 説明の日本語訳
{
	local ($line) = @_;
	...
	if ($line =~ s/\s+#//) {
		@str = split("#");
		if (@str >= 5) {
			# 「# Lines starting with '#' are considered comments.」への対応
			printf OUT "%s#%s#%s\n", $str[0], $str[3], $str[4];
		} else {
			printf OUT "%s#%s\n", $str[0], $str[2];
		}
...
for ($i=0; $i<$cnt; $i++) {
	...
	if ( (/^-----------------------------------/ || /^\.\.\.\.\.\.\.\.\.\.\.\.\.\.\.\.\./) 
	  && ( $lines[$i+1] !~ /^\/\// ) ) {
		...
		# ブロック内の時だけ、文字を出力 (ただし、ブロック専用のロジックで出力する)
		if ($flg == 1) {
			putContStr $_;
		}
	}
